...
function isPalindrome(str) {
  const strLowerNoGap = str.replaceAll(' ', '').toLowerCase();
  const reverseStr = strLowerNoGap.split('').reverse().join('');
  return reverseStr === strLowerNoGap;
}
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    if (theLongest.length < arr[i].length) {
      theLongest = arr[i];
    }
  }
  return theLongest;
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (alphabet.includes(str.charAt(i))) {
      result += alphabetROT13.charAt(alphabet.indexOf(str.charAt(i)));
    } else {
      result += str.charAt(i);
    }
  }
